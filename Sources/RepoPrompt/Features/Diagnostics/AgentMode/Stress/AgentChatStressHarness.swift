        ...
        private func loadPersistedCodexFixtureBashReplay(
            ...
            let replayableEvents = events.filter { event in
                ...
                case .toolCall, .commandExecutionRunning:
                    true
                case .toolResult, .turnStarted, .turnCompleted, .assistantDelta, .reasoningDelta, .tokenUsage, .contextCompacted, .approvalRequest, .permissionsRequest, .requestUserInput, .mcpElicitationRequest, .serverRequestIssue, .error, .system:
                    false
                }
            }
