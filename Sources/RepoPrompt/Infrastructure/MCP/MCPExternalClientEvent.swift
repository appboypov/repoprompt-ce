...
/// Represents an error event from an external MCP client (like the CLI) that failed to connect.
/// These events are written to disk by the CLI and read by the app to surface diagnostics
/// even when network communication is impossible.
struct MCPExternalClientEvent: Codable, Equatable {
    /// Source of the event
    ...
    /// Specific error code mapping to CLI error types
    ...
    let clientName: String? // e.g., "Cursor", "Codex", "Claude Desktop"
